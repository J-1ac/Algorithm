...
int gm(const vector<int> &a, const vector<int> &b){
    int d=0;
    d+=abs(a[0]-b[0]);
    d+=abs(a[1]-b[1]);
    return d;
...
int calb(Node cn, int n, const vector<vector<int>>& dis){
    int b=cn.d, i=1, min=1234567890;
    while(i<2*n+1){
        bool possible=false;
...
void bb(int n, const vector<vector<int>>& dis){
    ...
    while(!pq.empty()){
        ...
        while(i<2*n+1){
            bool possible=false;
            if(v.c<2 && i%2==1 && count(v.p.begin(), v.p.end(), i)==0) possible=true;
            else if(i%2==0 && count(v.p.begin(), v.p.end(), i-1)!=0 && count(v.p.begin(), v.p.end(), i)==0) possible=true;
            if(possible){
                if(i%2) u.c=v.c+1;
                else    u.c=v.c-1;
                u.l=v.l+1;
...
int main(){ 
    int n;
    cin >> n;
    vector<vector<int>> nodes(n, vector<int>(4,0)), dis(2*n+1,vector<int>(2*n+1,0)); vector<int> n1(2), n2(2);
    for(int i=0; i<n; i++){
